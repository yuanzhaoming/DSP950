...
int get_link_status(void)
{
	...
	if(enc28j60_phy_read(spi_device, PHSTAT1) & 0x4)
	  	i_ret = 1;
	else
		i_ret = 0;
	enc28j60_unlock(&enc28j60_dev);
...
#endif /* RT_USING_SPI1 */
	enc28j60_attach("spi-elm");

#if 0
  {
  	NVIC_InitTypeDef NVIC_InitStructure; 
  	//配置以太网芯片(enc28j60)的中断信号(PE4)
  	GPIO_InitTypeDef GPIO_InitStructure;
...
{
#ifdef  RT_USING_LWIP  //enc28j60中断使用PE4
